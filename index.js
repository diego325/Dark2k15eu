...
const BotName = 'ESTER - BOT DARK MODZ'; 
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\n*EU SOU A ESTER, BOT DO GRUPO*\nEspero que goste do grupo🥰 \nNÃO SEJA UM GHOST\n\n\nNAO LIGUE PRA MINHA VOZ DE HOMI😳`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] DARK MODZ? Este é um recurso especial para o DARK MODZ ❌',
					ownerB: '[❗] DARK MODZ? Este é um recurso especial para o DARK MODZ ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de DARK MODZ. Registre-se para fazer amizade com o bot DARK MODZ por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar DARK|15\n\n──「 ESTER BOT 」──`,
			...
			const ownerNumber = ["5521982310081@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5521982310081@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5521982310081@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5521982310081@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5521982310081@s.whatsapp.net"]
            ...
            try {
                ...
                const amountXp = Math.floor(Math.random() * 20) + 5000
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos KKKKKKKK tchau otário 😔🤙")
		}, 0)
	}
            ...
            case 'bodoh':
				...
				case 'daftar':
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`O registro foi bem sucedido com SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`Sobre ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${empurrão}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Idade]: ${umure}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS ESTER BOT 🐒* 」\n\`\`\``
					no = 0
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, no momento da conversão ${tipe} para figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador @${nomor.split("@s.whatsapp.net")[0]}\nsekarang *APENAS ADMINISTRADOR/ES* pode enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
...
// recursos anime, random @DARK MODZ domina irmão
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "DARK MODZ", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o DARK MODZ ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMIN BOT ESTER - DARK MODZ 🇧🇷* 」\n*│+ wa.me/5521982310081*╰──────*「 *DARK MODZ* 」*────*\n\n*_SE QUER SER ADMIN DO BOT ESTER_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'jaine':
				    ...
				    client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*Linda e maravilhosa:*\n\n gostosa'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'admins':
					...
					break
                                case 'linkgp':
                                        ...
                                        reply('https://chat.whatsapp.com/'+linkgp)
                                        break
                                case 'leave':
                                        if (!isGroup) return reply(mess.only.group)
